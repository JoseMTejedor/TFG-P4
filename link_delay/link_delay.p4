...
// Cabecera personalizada TMEA
header measurement_t {
    bit<32> in_time;
    bit<32> accum_time; // Campo para el retardo acumulado
    bit<48> prev_egress_t; //Egress timestamp del anterior conmutador
    bit<32> link_time; //Retardo desde que entro en el egress del conmutador anterior hasta que ha llegado al ingress actual
}
...
control MyEgress(inout headers hdr,
    ...
    apply { 
        ...
        if (hdr.measurement.isValid()) {
            ...
            //Calculamos el retardo entre el ingress y el egress
            bit<32> delta = (bit<32>)(current_egress - current_ingress);
            hdr.measurement.in_time = delta;

            //Calculamos el Link Delay 
            // Si el dato de prev_egress es distinto de 0 (dato de inicilización) se hace el calculo
            // Se calcula restando el egress_timestamp del anterior conmutador menos el ingress_timestamp de nuestro conmutador actual
