...
// Tipos de digest enviados al controlador.
const bit<8> DIGEST_NEW_FLOW = 8w1;
const bit<8> DIGEST_FLOW_ALIVE = 8w2;
// Puerto de telemetría para los switches P4
const bit<9>  TELEMETRY_PORT = 9w5;            
...
// Informacion enviada al controlador mediante digest.
// Avisar al controlador del inicio de una conexión de pares o su mantenimiento.
...
struct metadata {
    ...
    @field_list(1) ip4Addr_t src_ip_backup;
    @field_list(1) ip4Addr_t dst_ip_backup;

    // Flags internos.
    bit<1> first_p4_switch;
    bit<1> send_new_flow_digest;
    bit<1> send_alive_digest;
}
...
control MyIngress(inout headers hdr,
    ...
    // Acción usada cuando una ruta dinámica está en modo probation.
    // Reenvía el paquete igual que ipv4_forward, pero marca que hay que mandar
    // un digest FLOW_ALIVE al controlador.
    action ipv4_forward_probe(macAddr_t dstAddr, egressSpec_t port) {
        standard_metadata.egress_spec = port;
        hdr.ethernet.srcAddr = hdr.ethernet.dstAddr;
        hdr.ethernet.dstAddr = dstAddr;
        hdr.ipv4.ttl = hdr.ipv4.ttl - 1;

        meta.send_alive_digest = 1w1;
    }
    ...
    // Tabla de rutas dinámicas por par origen-destino.
    // Si hay entrada, se usa esta ruta.
    // Si no hay entrada, el paquete cae por ipv4_lpm como ruta por defecto.
    table flow_routing {
        key = {
    ...
    apply {
        ...
        // Inicializamos los flags internos de control.
        meta.first_p4_switch = 1w0;
        meta.send_new_flow_digest = 1w0;
        meta.send_alive_digest = 1w0;

        // Cargamos el ID del switch nada más entrar al pipeline de ingress.
        switch_info.apply();

        if (standard_metadata.ingress_port == TELEMETRY_PORT){
            drop();
        } else if (hdr.ipv4.isValid()) {

            // Si el paquete viene del host, no lleva la cabecera de medición, la añadimos.
            // Esto ocurre en el primer switch P4 que procesa el paquete.
            if (!hdr.measurement.isValid()) {
                //Guardamos que es el primer switch P4 del camino.
                meta.first_p4_switch = 1w1;
            ...
            // Primero intentamos usar una ruta dinámica por par src-dst.
            // Si no existe, avisamos al controlador mediante digest y usamos
            // la ruta estática por defecto.
            if (flow_routing.apply().hit){
                //flow_routing ya ha elegido el puerto de salida.
            } else {
                if (meta.first_p4_switch == 1w1){
                    //Activamos el flag para que mande digest avisando de la nueva conexión.
                    meta.send_new_flow_digest = 1w1;
                }
                //Ruta por defecto.
                ipv4_lpm.apply();
            }
            
            // Solo enviamos digest al controlador si se trata de primer switch P4.
            // Se evita que los switch intermedios avisen del mismo flujo, para que
            // no se sobrecargue la red del controlador.
            if (meta.first_p4_switch == 1w1) {

                if (meta.send_new_flow_digest == 1w1) {
...
control MyEgress(inout headers hdr,
    ...
    apply { 
        ...
        if (standard_metadata.instance_type == 0) {
            ...
            if (hdr.measurement.isValid()) {
                ...
                if(rand_val <= SAMPLE_PERCENT){
                    ...
                    meta.src_ip_backup = hdr.ipv4.srcAddr;
                    meta.dst_ip_backup = hdr.ipv4.dstAddr;
        ...
        // sino que se transforma en un paquete UDP de telemetría hacia el colector.
            ...
            hdr.telemetry.src_ip = meta.src_ip_backup;
            hdr.telemetry.dst_ip = meta.dst_ip_backup;
            ...
            // Longitud UDP: 8 UDP + 32 telemetry = 40 bytes.
            hdr.udp.length_ = 40;   
            ...
            // 14 Ethernet + 20 IPv4 + 8 UDP + 32 telemetry = 74 bytes.
            truncate(74);
        }   
    }
}
